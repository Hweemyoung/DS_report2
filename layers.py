...
class LinearSeq(nn.Module):
    def __init__(self, in_features, out_features_list, bias_list=True, batch_normalization_list=True,
        ...
        # ])
        self.layers = nn.ModuleList([])
        for (i, o, b, bn, a, d) in zip([in_features] + out_features_list[:-1], out_features_list, bias_list, batch_normalization_list,
                activation_list, dropout_list):
            self.layers.append(LinearNorm(i, o, b, bn, a, d))
